...
const char szCommandLineHelpMsg[] = "Service Manager for Windows " SRVMAN_VERSION_STR "\r\n\
...
//! Expands a possibly relative path against the current directory.
/*! The BazisLib FilePath class this code was originally written against had a
	ConvertToAbsolute() method. The current Path API is stateless, so the
	expansion is done directly via the Win32 API. */
static String MakeAbsolutePath(const String &path)
{
	TCHAR tszFull[MAX_PATH * 4];
	DWORD done = GetFullPathName(path.c_str(), __countof(tszFull), tszFull, NULL);
	if (!done || (done >= __countof(tszFull)))
		return path;
	return ConstString(tszFull);
}
...
ActionStatus CmdlInstallService(HANDLE hStdOut, LPCTSTR lpBinPath, LPCTSTR lpSvcName, LPCTSTR lpDisplayName, DWORD dwType, DWORD dwStart, bool Overwrite)
{
	...
	if (!lpSvcName)
	{
		...
		if (pArgv && pArgv[0])
			strTmp = BazisLib::Path::GetFileNameWithoutExtension(ConstString(pArgv[0]));
		else
			strTmp = BazisLib::Path::GetFileNameWithoutExtension(ConstString(lpBinPath));
		if (pArgv)
			LocalFree(pArgv);
...
ActionStatus CmdlRunBinary(HANDLE hStdOut, LPCTSTR lpBinary, LPCTSTR lpSvcName, bool Copy, bool Overwrite, unsigned StopAfter)
{
	...
	if (Copy || !lpSvcName)
	{
		...
		if (pArgv && pArgv[0])
			fpSrc = ConstString(pArgv[0]);
		else
			fpSrc = ConstString(lpBinary);
		if (pArgv)
			LocalFree(pArgv);

		if (!lpSvcName)
		{
			strTmp = BazisLib::Path::GetFileNameWithoutExtension(fpSrc);
			lpSvcName = strTmp.c_str();
		}
		...
		if (Copy)
		{
			...
			fp = BazisLib::Path::Combine(ConstString(tsz), ConstString(_T("drivers")));
			fp = BazisLib::Path::Combine(fp, BazisLib::Path::GetFileName(fpSrc));
			if (BazisLib::File::Exists(fp) && !Overwrite)
...
int ConsoleMain( int argc, LPWSTR *pArgv, bool ConsoleExisted )
{
	...
	if (cmdArgs.Params().size() >= 2)
	{
		...
		{
			String fp = ConstString(cmdArgs.Params()[1].c_str());
			DWORD dwDefType = SERVICE_WIN32_OWN_PROCESS | SERVICE_INTERACTIVE_PROCESS;
			if (!_tcsnicmp(BazisLib::Path::GetExtensionExcludingDot(fp).c_str(), _T("sys"), 3))
		...
		{
			String fp = BazisLib::Path::Combine(
				BazisLib::Path::GetSpecialDirectoryLocation(dirDrivers),
	...
	if (Parsed)
	{
		BazisLib::DynamicStringA strResult = StringToANSIString(st.GetMostInformativeText());
