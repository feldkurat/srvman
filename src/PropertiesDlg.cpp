...
LRESULT CPropertiesDlg::OnSettingChange( UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/ )
{
	// The broadcast reaches this dialog too, and it can be modal for a long time.
	// Every top-level window re-applies unconditionally rather than keying off
	// RefreshFromSystem()'s return value: whichever window is handed the broadcast
	// first would otherwise absorb the state change and leave the others stale.
...
LRESULT CPropertiesDlg::OnBnClickedBrowse(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	if (!::GetWindowTextLength(GetDlgItem(IDC_INTERNALNAME)))
	{
		SetDlgItemText(IDC_INTERNALNAME, BazisLib::Path::GetFileNameWithoutExtension(str).c_str());
		String ext = BazisLib::Path::GetExtensionExcludingDot(str);
...
unsigned CPropertiesDlg::GetNonServiceProcessCommandLine( LPCTSTR lpSrvManCommandLine )
{
	...
	if (!argv)
		return 0;
	unsigned off = 0;
	String moduleName = BazisLib::Path::GetFileNameWithoutExtension(ConstString(argv[0]));
	if ((argc >= 2) && !_tcsicmp(moduleName.c_str(), _T("srvman")) && !_tcsnicmp(argv[1], _T("/RunAsService:"), 14))
	{
